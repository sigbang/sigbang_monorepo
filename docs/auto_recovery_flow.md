...
- **자동 복구 (고 CPU 인스턴스 교체)**
  - Lambda 코드: `lambda/terminate_hot_instance.py`
  - Lambda 함수: `aws_lambda_function.terminate_hot_instance`
...
- **Target**
  - `aws_lambda_function.terminate_hot_instance`

### 4. Lambda 자동 복구 로직 (`terminate_hot_instance.py`)

- **환경 변수**
  - `ASG_NAME`: 대상 Auto Scaling Group 이름 (`api_asg`)
  - `CPU_THRESHOLD`: `"95"`  
...
- **의도**
  ...
  - 너무 공격적으로 동작하지 않도록:
    - 80%/10분 + 95%/인스턴스 + `MIN_IN_SERVICE` 등의 조건으로 방어막을 겹겹이 둔다.
...
- **CloudWatch / Lambda 설정 점검**
  - Lambda 함수:
    ...
    - 환경 변수:
      - `ASG_NAME` 가 실제 API ASG 이름과 일치하는지
      - `CPU_THRESHOLD = 95`, `LOOKBACK_MIN = 10`, `MIN_IN_SERVICE = 2`
  - EventBridge Rule:
    - `sigbang-api-cpu-hot-instance-rule` 존재 여부
    - Target 에 Lambda 가 연결되어 있는지
  - CloudWatch 알람:
...
- **1) 메일 알람 확인**
  - `api_cpu_hot`, `api_cpu_high`, `api_alb_5xx_high` 중 어떤 알람이 언제 발생했는지 확인.

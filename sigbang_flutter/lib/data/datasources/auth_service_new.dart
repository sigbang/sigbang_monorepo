...
class AuthService {
  ...
  Future<UserModel> signInWithEmail(String email, String password) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final exp = JwtUtils.getExpiryEpochSeconds(loginResponse.tokens.accessToken);
        if (exp != null) {
          await SecureStorageService.saveAccessTokenExpiryEpoch(exp);
        } else if (loginResponse.tokens.expiresIn != null) {
          final now = DateTime.now().millisecondsSinceEpoch ~/ 1000;
          await SecureStorageService.saveAccessTokenExpiryEpoch(now + loginResponse.tokens.expiresIn!);
        }
        await SecureStorageService.saveUserInfo(json.encode(loginResponse.user.toJson()));
        return loginResponse.user;
      } else {
  ...
  Future<void> resendVerification(String email) async {
    try {
      await _apiClient.dio.post('/auth/email/resend', data: {'email': email});
    } on DioException catch (e) {
      throw Exception('인증 메일 재발송 실패: ${e.response?.data?['message'] ?? e.message}');
    }
  }
  ...
  Future<void> requestPasswordReset(String email) async {
    try {
      ...
      throw Exception('재설정 메일 요청 실패: ${e.response?.data?['message'] ?? e.message}');
    }
  }
  ...
  Future<void> resetPassword(String token, String newPassword) async {
    try {
      ...
      throw Exception('비밀번호 재설정 실패: ${e.response?.data?['message'] ?? e.message}');
    }
  }
  ...
  Future<void> changePassword(String currentPassword, String newPassword) async {
    try {
      await _apiClient.dio.patch('/auth/password', data: {
        'currentPassword': currentPassword,
        'newPassword': newPassword,
      });
      // 서버가 모든 리프레시 토큰을 폐기하므로 로컬 로그아웃 처리
      await SecureStorageService.clearAll();
    } on DioException catch (e) {
      throw Exception('비밀번호 변경 실패: ${e.response?.data?['message'] ?? e.message}');
    }
  }

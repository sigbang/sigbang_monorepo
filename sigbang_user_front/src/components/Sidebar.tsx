...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden sm:flex sm:flex-col sm:w-[200px] sticky top-[84px] self-start h-[calc(100vh-160px)] px-4 pt-12 gap-6" aria-label="사이드바 내비게이션">      
      <nav className="flex flex-col gap-3 text-[18px]">
        ...
        <Link className={`flex items-center gap-2 rounded text-[#6b7280] hover:text-black ${isActive('/recipes/import') ? 'text-black' : ''}`} href="/recipes/import" aria-current={isActive('/recipes/import') ? 'page' : undefined}><PlusIcon size={24} filled={isActive('/recipes/new')} />{t('nav.create')}</Link>
      </nav>
